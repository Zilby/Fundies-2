...
public class Deque<T> implements Iterable<T> {
    ...
    public Iterator<T> iterator() {
        return new ForwardDequeIterator<T>(this.header);
        // return new ReverseDequeIterator<T>(this.header);
    }
...
class ForwardDequeIterator<T> implements Iterator<T> {
    ...
    ForwardDequeIterator(ANode<T> curr) {
        this.curr = curr;
    }
...
class ReverseDequeIterator<T> implements Iterator<T> {
    ANode<T> curr;

    ReverseDequeIterator(ANode<T> curr) {
        this.curr = curr;
    }

    // returns true if there's at least one value left in this iterator
    public boolean hasNext() {
        return curr.hasNext2();
    }

    // returns the next value and advances the iterator
    public T next() {
        T temp = this.curr.prev.getData();
...
class ExamplesDeque {
    ...
    void testDequeIteration(Tester t) {
